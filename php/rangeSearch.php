...
/**
 * Endpoint: Buscar puntos dentro de un rango en el KD-Tree.
 *
 * Metodo: POST
 * Parametros: cpu_usage, memory_usage, hana_memory, dialog_response_time, 
 *             work_processes, enqueue_locks, radius
 * Retorna: JSON con los puntos encontrados y metricas.
 */
...
try {
    ...
    if (!$input || !isset($input['radius'])) {
        http_response_code(400);
        echo json_encode([
            'success' => false,
            'error' => 'Datos de entrada invalidos. Se requiere radio (radius).',
        ]);
        exit;
    }

    $radius = (float)$input['radius'];
    $result = $builder->searchRange($input, $radius);
